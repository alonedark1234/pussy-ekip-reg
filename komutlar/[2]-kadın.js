...
let Name = args[1]
let Age = args[2]
if(!member) return message.channel.send(new MessageEmbed().setDescription(`Argümanları lütfen doğru giriniz örnek : .k @Passenger/ID İsim Yaş`).setAuthor(message.author.tag, message.author.avatarURL({dynamic: true})).setColor(settings.renk.kirmizi)).then(x => x.delete({timeout: 6500}));
...
.setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
.setDescription(`${member} üyesi ${message.author} tarafından kayıt edildi. \n\nÜye artık <@&${settings.roller.kadınrol1}>, <@&${settings.roller.kadınrol2}>, <@&${settings.roller.kadınrol3}> rollerine sahip. \nKullanıcının ismi \`${Tagisim}\` olarak güncellendi.`)
.setFooter(`Toplam kayıtların: ${kayıtlar}`)
.setColor(settings.renk.mor))

client.channels.cache.get(settings.kanallar.genelchat).send(`${member} aramıza katıldı, sunucumuza hoş geldin umarım keyifli vakit geçirirsin !`)
