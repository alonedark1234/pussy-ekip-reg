...
let Name = args[1]
let Age = args[2]
if(!member) return message.channel.send(new MessageEmbed().setDescription(`Argümanları lütfen doğru giriniz örnek : .e @Passenger/ID İsim Yaş`).setAuthor(message.author.tag, message.author.avatarURL({dynamic: true})).setColor(settings.renk.kirmizi)).then(x => x.delete({timeout: 6500}));
...
.setColor(settings.renk.passmavi))
